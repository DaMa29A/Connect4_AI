...
#     agent1 = DQNAgent(env, deterministic=True) # Player 1 (X)
#     #agent2 = PPOAgent(env, deterministic=False) # Player 2 (O)
#     agent2 = RandomAgent(env) # Player 2 (O)
#     #agent2 = RuleBasedL2Agent(env) # Player 2 (O)
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        ...
        for game_num in range(1, NUM_GAMES + 1):
            ...
            while not done:
                ...
                if defensive_flag:
                    write_defensive_success(f, player_symbol)
                write_board(f, format_board(obs))
